...
\begin{equation*}
\hat{\boldsymbol{\beta}}_i = \underset{\boldsymbol{\beta}_i}{\text{arg min}}\left[\frac{1}{2}\boldsymbol{\beta}'_i(\textbf{G}+\lambda_0 \textbf{I})\boldsymbol{\beta}_i - \textbf{G}'_i\boldsymbol{\beta}_i\right]
\end{equation*}

where $\lambda_0=\sigma_\varepsilon^2/\sigma_u^2=(1-h^2)/h^2$  is the ratio of the error to the genetic variance, which can be expressed in terms of the **heritability**, $h^2$. The solution to the above problem can be shown to be 

\begin{equation*}
\hat{\boldsymbol{\beta}}_i = (\textbf{G}+\lambda_0 \textbf{I})^{-1}\textbf{G}_i 
\end{equation*}
...
\begin{equation*}
\tilde{\boldsymbol{\beta}}_i = \underset{\boldsymbol{\beta}_i}{\text{arg min}}\left[\frac{1}{2}\boldsymbol{\beta}'_i(\textbf{G}+\lambda_0 \textbf{I})\boldsymbol{\beta}_i - \textbf{G}'_i\boldsymbol{\beta}_i + \lambda\sum_{j=1}^n|\beta_{ij}|\right]
\end{equation*}
...
The next script can be used to create a plot showing (for a single trn-tst partition) the subset of points in the support set, for each individual being predicted. This plot can be made through the function `plotNet` from the SFSI package.
